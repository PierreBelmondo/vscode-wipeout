 ...
 */
function instrumentCoverage(byteLength: number): Uint8Array {
...
function coverage(filename: string) {
  ...
  console.log(`  objects     : ${rcs.objects.length}`);
  console.log(`  materials   : ${rcs.materials.length}`);
  ...
  const gaps: { offset: number; length: number; zero: boolean }[] = [];
  for (let i = 0; i < size; ) {
    if (hits[i]) { i++; continue; }
    const start = i;
    while (i < size && !hits[i]) i++;
    let zero = true;
    for (let k = start; k < i; k++) if (buffer[k] !== 0) { zero = false; break; }
...
function main(args: string[]) {
  if (args.length == 4 && args[2] == "coverage") {
    coverage(args[3]);
    return;
  }
  if (process.argv.length != 3) {
    console.log("Usage: ts-node scripts/rcsmodel.ts <filename.rcsmodel>");
    console.log("       ts-node scripts/rcsmodel.ts coverage <filename.rcsmodel>");
    return;
  }
  ...
  if (config.meshes) {
    for (const object of rcs.objects) {
      ...
      if (object.mesh instanceof RcsModelMesh1) {
        ...
        output.log(`Part table offset:  0x${object.header.part_table_offset.toString(16)}`);
        output.br();
